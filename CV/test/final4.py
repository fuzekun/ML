...
def preHandle3(image) :                           #去除边框的处理
    ...
    # 去除边框
    tmpb = []
    bfs(ret, 0, 0, vis, tmpb)
    changeTo0(ret, tmpb)

    return  ret

def get_letter_area(contours):

    for i in range(0, len(contours)):
        min__rect = cv.minAreaRect(contours[i])
        if min(min__rect[1])>5:
             if float(max(min__rect[1]) / min(min__rect[1])) <= 25 and float(max(min__rect[1]) / min(min__rect[1])) >= 7 and \
...
def getChars(areaT, contImg) :
    ...
    :return:        字符的联通区域
    """
    wer, contours, t = cv.findContours(contImg, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
    idx = get_letter_area(contours)
...
def readTrain(trainingMat, trainingFileList) :                  #获取训练集合,返回训练集和标签
    ...
    for i in range(m):
        ...
        fr = open('TrainData2/' + fileNameStr, "r")              # 读取文件
...
def run() :
    ...
    trainingFileList = os.listdir('TrainData2')

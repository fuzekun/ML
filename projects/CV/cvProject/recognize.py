...
def columnCut(img):                         #图片进行列分割
    ...
    for i in range(min(len(start), len(end))):
        ...
        imgi = solveWImg(imgi)
        if len(img) > 0 and len(img) *  len(imgi[0]) > 50:        #有可能截取出来噪声点
            # show("c_" + str(i), imgi)
            imgs.append(imgi)
...
if __name__ == '__main__':
    ...
    file = open('wronglne.txt', 'r')
    wrongList = file.readlines()
    file.close()

    for wrongName in wrongList:

        wrongName = wrongName.strip()
        print(wrongName)
        img = preHandle('images/' + wrongName)
        degree = findConnection(img, 50)
        rotated = transpos(img, degree)                 #将图像进行旋转
        rec = getBounder(rotated, 50)                   #获取旋转图像的上下边界

        img = cutLine(rotated, rec)
        imgs = columnCut(img)

        if (len(imgs) == 0) or (len(imgs[0]) == 0) :
            print("分割出错")
            continue
        ...
        #     show(str(i), imgs[i])
    # draw(img)
    # cv.imwrite('X.jpg', img[len(img) - 1])

...
def run() :
    ...
    for img_i in range(testTime):
        ...
        line = getLine2(img, 0)           # 行分割
        if len(line) == 0 :
            line = getLine2(img, 1)       # 去除边框在分割
        if len(line) == 0 :
            line = getLine2(img, 2)       # 转化颜色在分割
        if len(line) == 0:
            print("行分割出错")
            cntH += 1
            continue

        imgName = 'Models/' + pic.split('.')[0] + '.txt'
        # print(imgName)

        changToTxt(imgName, line)       # 行分割出来的图片制作成模板


    print('行分割出错' + str(cntH))

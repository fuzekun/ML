...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--my-arg', type=int, help='Description of my argument')
...
class Smoke_File_Detector():
    def __init__(self):
        ...
        # parser.add_argument('--weights', nargs='+', type=str, default=r'weights/smoke.pt', help='model.pt path(s)')
        ...
        model = cache.get('fireDetectModel')
        if model != None:
            self.model = model
        else:
            self.model = self.model = attempt_load(self.weights, map_location=self.device)
            cache.set('fireDetectModel', self.model, timeout=3600)
        ...
        # self.half = self.device.type != 'cpu'  # half precision only supported on CUDA
        # self.model = attempt_load('weights/smoke.pt', map_location=self.device)
    ...
    def detect_test(self,test_list):
        ...
        pred = self.model(batch_img, augment=self.augment)[0]
...
if __name__ == "__main__":
    ...
    print(arr)
    img = det.solveFireImg(img)
    cv2.imwrite('fireLabel.jpg', img)
    # print(arr)
